...
public partial class ShellStatement : Executable
{
	...
	class TerminalControlSequenceProcessor
	{
		// Really Terminal Control Sequence "Ignorer" :-D
		...
		enum State
		{
			...
			InDesignateCharacterSetSequence, // ESC ( x
		...
		public void ProcessByte(byte b, Action<byte> emit)
		{
			switch (_state)
			{
				...
				case State.HaveOriginatorEscape:
					switch (b)
					{
						case (byte)'[': _state = State.InVTControlSequence; break;
						case (byte)'(': _state = State.InDesignateCharacterSetSequence; break;
				...
				case State.InDesignateCharacterSetSequence:
					_state = State.Normal;
					break;
