...
public class BasicParser(IdentifierRepository identifierRepository)
{
	...
	internal IEnumerable<CodeLine> ParseCodeLines(IEnumerable<Token> tokenStream, bool ignoreErrors = false)
	{
		...
		// If we've found no tokens, then if there's nothing on the line at all, return an
		...
		if (((line.Statements.Count > 0) || buffer.Any()) && !lineConsumed)
		{
			if (lastToken == null)
