...
{
	public HelpFile Load(string path)
	{
		...
		using (var reader = new BinaryReader(stream))
		{
			...
			var ret = new HelpFile();

			var attributes = reader.ReadUInt16();

			ret.CaseSensitiveContextStrings = (attributes & 1) != 0;
	...
	HelpFileTopic DecodeTopic(byte[] compressedData, HuffmanTreeNode? huffmanTree, byte[][]? keywords)
	{
		...
		return HelpFileTopic.Parse(dataSpan);
	}

	private Span<byte> HuffmanDecompress(ReadOnlySpan<byte> dataSpan, HuffmanTreeNode huffmanTreeRoot)
	{
		int nextBit = 128;
	...
	private Span<byte> KeywordDecompress(ReadOnlySpan<byte> dataSpan, byte[][] keywords)
	{
		var output = new List<byte>();
