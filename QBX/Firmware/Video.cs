...
public partial class Video(Machine machine)
{
	...
	public bool SetMode(int modeNumber, bool clearVRAM)
	{
		...
		array.OutPort2(
			...
			unchecked((byte)(
			(mode.OddEvenScans ? GraphicsRegisters.GraphicsMode_HostOddEvenRead : 0) |
		...
		array.OutPort2(
			...
			(mode.OddEvenColumns ? GraphicsRegisters.MiscGraphics_ChainOddEven : 0) |
		...
		array.OutPort2(
			...
			(mode.OddEvenColumns ? 0 : SequencerRegisters.SequencerMemoryMode_OddEvenDisable) |
	...
	void InitializeVisualLibrary(ModeParameters modeParams)
	{
		...
		{
			if (modeParams.OddEvenScans)
				SetVisualLibrary(new GraphicsLibrary_1bppInterleaved(machine));
			else
	...
	public void LoadCGAPalette(int cgaPalette = 1, bool monochrome = false, bool intensity = false, bool reloadDAC = false)
	{
		if (reloadDAC)
		{
			...
			// Copied from DOSBox-X, it's probably doing the right thing.

			// The first 64 entries are copied straight out of the standard
			// EGA attribute mappings with repetition. Colours 0-7 occur
			// twice, followed by colours 56-63, twice, producing 32 colours,
			// and then the entire sequence is repeated, filling 64 slots.

			for (int paletteRepeat = 0; paletteRepeat < 2; paletteRepeat++)
			{
				for (int lineRepeat = 0; lineRepeat < 2; lineRepeat++)
				{
		...
		if (monochrome)
		{
			...
			machine.GraphicsArray.AttributeController.Registers[1] = 63;
		}
		else
		{
			switch (cgaPalette)
